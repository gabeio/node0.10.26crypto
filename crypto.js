...
(function() {
  var argv, crypto, de, decrypted, en, encrypted, encryption;
  ...
  if (argv.pass) {
    ...
    if (!argv.encrypt && !argv.decrypt) {
      console.log('use --encrypt or --decrypt').exit;
    }
    if (!argv.bytes) {
      console.log('we need something to encrypt or decrypt, use --bytes <whatever here>').exit;
    }
    en = crypto.createCipher(encryption, argv.pass);
    de = crypto.createDecipher(encryption, argv.pass);
    encrypted = "";
    decrypted = "";
    if (argv.encrypt && argv.bytes) {
      encrypted += en.update(argv.bytes, 'utf8', 'base64');
      encrypted += en.final('base64');
      console.log(encrypted);
    }
    if (argv.decrypt && argv.bytes) {
      de.on('error', function(e) {
        return console.error('looks like you have the wrong key.');
      });
